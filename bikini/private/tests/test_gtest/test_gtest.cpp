...
TEST(base_random, set_seed) {
	bk::random_<0>::set(12345);
	bk::uint l_r0 = bk::random_<0>::get();
	bk::random_<0>::get();
	bk::random_<0>::get();
	bk::random_<0>::get();
	bk::random_<0>::set(12345);
	bk::uint l_r1 = bk::random_<0>::get();
	EXPECT_EQ(l_r0, l_r1);
}
TEST(base_random, get_range) {
	//
	bk::uint l_umax = 100;
	bk::uint l_u0 = bk::random_<0>::get(l_umax);
	EXPECT_LE(l_u0, l_umax);
	//
	bk::sint l_smin = -100, l_smax = -100;
	bk::sint l_s0 = bk::random_<0>::get(l_smin, l_smax);
	EXPECT_GE(l_s0, l_smin);
	EXPECT_LE(l_s0, l_smax);
	//
	bk::real l_rmin = -100, l_rmax = 100;
	bk::real l_r0 = bk::random_<0>::get(l_rmax);
	EXPECT_LE(l_r0, l_rmax);
	//
	bk::real l_r1 = bk::random_<0>::get(l_rmin, l_rmax);
